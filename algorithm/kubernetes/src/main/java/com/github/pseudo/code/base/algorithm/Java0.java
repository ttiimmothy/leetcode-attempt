package com.github.pseudo.code.base.algorithm;
...
public class Java0 {
  ...
  public void mergeSort_1(int[] nums, int low, int high) {
    if (low < high) {
      int mid = low + (high - low) / 2;
      mergeSort_1(nums, low, mid);
      mergeSort_1(nums, mid + 1, high);
      int n1 = mid - low + 1;
      int n2 = high - mid;
  ...
  public void heapify_1(int[] nums, int maxNode, int length) {
    int largest = maxNode;
    int left = 2 * maxNode + 1;
    int right = 2 * maxNode + 2;
    if (left  < length && nums[left] > nums[largest]) {
  ...
  public void heapSort_1(int[] nums) {
    int n = nums.length;
    // create max heap
    for (int i = n / 2 - 1; i >= 0; i--) {
      heapify_1(nums, i, n);
